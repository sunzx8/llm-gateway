...
#
# 使用:
#   bash train_ingest_grpo.sh [额外 slime 参数...]
#   FROZEN_MODEL_URL=http://host:8000/v1/chat/completions bash train_ingest_grpo.sh  # 外部 endpoint
# =============================================================================
...
EVAL_DATA="${VAL_DATA:-${EVAL_DATA:-${PROJECT_ROOT}/data/ingest_val.jsonl}}"

MEGATRON_ROOT="${MEGATRON_ROOT:-/root/Megatron-LM}"
SLIME_TRAIN_SCRIPT="${SLIME_TRAIN_SCRIPT:-/root/slime/train_async.py}"
MODEL_CONFIG_ARGS="${MODEL_CONFIG_ARGS:-}"
CUSTOM_RM_PATH="${CUSTOM_RM_PATH:-llm_gateway.rl.slime_train.tasks.ingest_reward.reward.reward_func}"
...
# 唯一标识
RUN_NAME="ingest_grpo_$(date '+%Y%m%d_%H%M%S')"
