...
def get_snapshot_session() -> SnapshotSession:
    ...
    data_root = os.environ.get(
        "RETRIEVE_SNAPSHOT_DATA_ROOT",
        os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "ingest_snapshots"),
    )
    with _cache_lock:
        if _snapshot_session is None or _snapshot_session.data_root != data_root:
            _snapshot_session = SnapshotSession(data_root=data_root)
