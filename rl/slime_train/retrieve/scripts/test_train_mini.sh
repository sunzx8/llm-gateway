#!/bin/bash
# =============================================================================
# Retrieve 消费 RL 测试训练脚本 — 小规模验证能否跑通梯度更新
#
# 使用方法：在 head 节点容器内执行:
#   bash slime_train/retrieve/scripts/test_train_mini.sh
#   bash slime_train/retrieve/scripts/test_train_mini.sh --task-version agent_loop
#
# 精简参数（相比完整训练）:
...
# 输出
RUN_NAME="retrieve_test_$(date '+%Y%m%d_%H%M%S')"
...
echo "============================================"
echo "Retrieve RL 测试训练"
echo "============================================"
echo "RUN_NAME:      ${RUN_NAME}"
echo "TRAIN_DATA:    ${TRAIN_DATA}"
echo "HF_CHECKPOINT: ${HF_CHECKPOINT}"
echo "CKPT_DIR:      ${CKPT_DIR}"
echo "TASK_VERSION:  ${MEMORY_RL_TASK_VERSION}"
echo "============================================"

# Step 0: 生成训练数据（如不存在）
if [ ! -f "${TRAIN_DATA}" ]; then
    echo ""
    echo "=== Step 0: 生成 retrieve 训练数据 ==="
    mkdir -p "$(dirname "${TRAIN_DATA}")"
    python3 "${PROJECT_ROOT}/convert_to_slime_format.py" \
        --input "${SNAPSHOT_DATA_ROOT}/rl_data.jsonl" \
        --output "${TRAIN_DATA}" \
        --eval_output "${PROJECT_ROOT}/data/rl_val.jsonl" \
        --eval_ratio 0.05
    echo "训练数据生成完成"
fi
...
# Reward QA 模型：如果传入 FROZEN_MODEL_URL，则直接复用外部 vLLM/OpenAI-compatible 服务。
echo ""
...
RUNTIME_ENV_JSON="{
  \"env_vars\": {
    ...
    \"no_proxy\": \"${no_proxy}\",
    \"RETRIEVE_SNAPSHOT_DATA_ROOT\": \"${SNAPSHOT_DATA_ROOT}\",
...
ray job submit --address="${RAY_ADDRESS}" \
    ...
    --apply-chat-template \
    --custom-rm-path llm_gateway.rl.slime_train.tasks.retrieve_reward.reward.reward_func \
    --num-rollout 10 \
    --rollout-batch-size 8 \
    --n-samples-per-prompt 4 \
    ...
    --sglang-router-request-timeout-secs 120 \
