#!/usr/bin/env python3
"""
SwanLab 实时训练监控脚本 — Retrieve Query Rewrite GRPO

同时解析：
1. slime 训练日志 (train.log / run*.log) → entropy, kl_loss, pg_loss, grad_norm, lr
2. reward 细分日志 (reward_metrics.jsonl) → r_retrieval_hit, r_format, r_query_quality, r_total
...
示例:
    python scripts/swanlab_monitor.py \
        --log-dir outputs/retrieve_test_XXXXXXXX/logs \
        --run-name "retrieve_grpo_v1"
"""
...
def monitor(
    ...
    rollout_group_rewards = []  # 累积一组 rollout 的 reward 用于计算 std
    ...
    while True:
        ...
        if train_log.exists():
            ...
            for line in new_lines:
                ...
                # Rollout 指标 (rewards, response_lengths)
        ...
        if reward_log.exists():
            ...
            for line in new_lines:
                ...
                if len(rollout_group_rewards) >= 8:
                    ...
                    # 计算 per-sample advantage (GRPO: (r - mean) / (std + 1e-6))
                    if group_std > 0:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="SwanLab 实时训练监控 — Retrieve GRPO")
    ...
    # 训练指标在 ray job 的主日志中（run*.log），不在 logs/train.log
    # 优先查找 log_dir 上级目录中最新的 run*.log
    train_log = log_dir / "train.log"
    if not train_log.exists():
        output_dir = log_dir.parent  # outputs/retrieve_test_XXX/
    ...
    # experiment_name 格式: {task_version}_retrieve_{时间戳}
    task_version = args.task_version or os.environ.get("MEMORY_RL_TASK_VERSION", "atomic_code_t2")
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    run_name = args.run_name or f"{task_version}_retrieve_{timestamp}"
    ...
    swanlab.init(
        ...
        config={
            ...
            "log_dir": str(log_dir),
            "reward_weights": {"r_retrieval_hit": 0.8, "r_format": 0.1, "r_query_quality": 0.1},
        },
    )
