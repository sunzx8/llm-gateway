#!/usr/bin/env python3
"""
SwanLab 实时训练监控脚本 — Consolidate GRPO

同时解析：
1. slime 训练日志 (train.log / run*.log) → entropy, kl_loss, pg_loss, grad_norm, lr
2. reward 细分日志 (reward_metrics.jsonl) → r_acc_delta, r_score_delta, r_after_score, r_format, r_total
...
def main():
    ...
    parser = argparse.ArgumentParser(description="SwanLab 实时训练监控 — Consolidate GRPO")
    ...
    # experiment_name 格式: {task_version}_consolidate_{时间戳}
    ...
    swanlab.init(
        ...
        config={
            ...
            "log_dir": str(log_dir),
            "reward_weights": {"r_acc_delta": 0.5, "r_score_delta": 0.25, "r_after_score": 0.15, "r_context_diff": 0.05, "r_format": 0.05},
        },
    )
