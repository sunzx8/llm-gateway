#!/bin/bash
# =============================================================================
# Consolidate 演进 RL 测试训练脚本 — 小规模验证能否跑通梯度更新
#
# 使用方法：在 head 节点容器内执行:
#   bash slime_train/consolidate/scripts/test_train_mini.sh
#   bash slime_train/consolidate/scripts/test_train_mini.sh --task-version agent_loop
#
# 精简参数（相比完整训练）:
...
# 输出
RUN_NAME="consolidate_test_$(date '+%Y%m%d_%H%M%S')"
...
if [ ! -f "${TRAIN_DATA}" ]; then
    echo "=== Step 0: 生成 consolidate 训练数据 ==="
    python3 "${PROJECT_ROOT}/convert_to_slime_format.py" \
        --input "${INPUT_RL_DATA}" \
        --data-root "${SNAPSHOT_DATA_ROOT}" \
        --output "${TRAIN_DATA}" \
        --eval_output "${PROJECT_ROOT}/data/consolidate_val.jsonl"
    echo "训练数据生成完成"
fi
...
RUNTIME_ENV_JSON="{
  \"env_vars\": {
    ...
    \"no_proxy\": \"${no_proxy}\",
    \"CONSOLIDATE_SNAPSHOT_DATA_ROOT\": \"${SNAPSHOT_DATA_ROOT}\",
...
ray job submit --address="${RAY_ADDRESS}" \
    ...
    --apply-chat-template \
    --custom-rm-path llm_gateway.rl.slime_train.tasks.consolidate_reward.reward.reward_func \
