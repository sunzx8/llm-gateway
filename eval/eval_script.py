...
::
    ...
    from agent_memory.eval.context_task_runner import (
        ContextTaskRunConfig,
        ContextTaskRunner,
    )
    from agent_memory.benchmarks.locomo_adapter import LoCoMoAdapter

    config = ContextTaskRunConfig(
        benchmarks=["locomo"],
        model="gpt-4o",
        provider="openai",
        subset_size=5,
    ...
    runner.register_benchmark(LoCoMoAdapter("data/locomo"))
        ...
        try:
            ...
            if interleaved:
                ...
                for sess_idx, session in enumerate(conv.sessions):
                    ...
                    logger.debug(
                        "  [conv=%s] ingest session %d/%d (%d msgs + %d questions, interleaved)",
                        conv.conv_id, sess_idx + 1, len(conv.sessions),
                        len(session.messages), min(n_append, len(sess_questions)),
                    )
                ...
                for sess_idx, session in enumerate(conv.sessions):
                    ...
                    # 拼接当前 session 对应的前 n 个 question 到 messages 末尾
                    # 标准模式下 questions 统一挂在 conv 上，按 session 索引分配
                    sess_questions_std: list[QAPair] = getattr(session, "questions", []) or []
                    n_append = self.config.ingest_append_questions
                    for qa in sess_questions_std[:n_append]:
                        messages.append({"role": "user", "content": qa.question})
                    logger.debug(
                        "  [conv=%s] ingest session %d/%d (%d msgs + %d questions)",
                        conv.conv_id, sess_idx + 1, len(conv.sessions),
                        len(session.messages), min(n_append, len(sess_questions_std)),
                    )
                    ingest_result = await self._call_ingest_api(
                        user_id=user_id,
                        session_id=session.session_id,
    ...
    async def _call_answer_api(
        ...
        *,
        ...
        try:
            ...
            elapsed = time.monotonic() - t0
            resp_text = e.response.text[:500] if hasattr(e.response, "text") else str(e)
            # 检测是否为内容策略拦截
            error_type = "http_error"
